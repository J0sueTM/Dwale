     ...
     char **argv)
{
  ...
  struct D_camera *camera = D_create_camera(D_PERSPECTIVE_PROJECTION, window, glm_rad(45.0f), 0.1f, 100.0f);
  ...
  struct D_surface *tux_surface =
    D_create_surface(D_SURFACE_RECTANGLE, GL_STATIC_DRAW, -0.4f, -0.8f, -0.4f, 0.8f, 0.4f, 0.8f, 0.4f, -0.8f);
  ...
  while (D_is_window_open(window))
  {
    ...
    glm_translate(tux_surface->model, (vec3){ 0.0f, sin(glfwGetTime()), 0.0f });
    glm_translate(tux_surface->model, (vec3){cos(glfwGetTime()), 0.0f, 0.0f });
